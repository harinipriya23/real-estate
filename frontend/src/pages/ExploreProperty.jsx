...
export const ExploreProperty = () => {
  const { filterDispatch, filterState, filteredProperties} = usePropertyContext();
  ...
  return (
    ...
    >
      {filterState.propertyType === "" ? <div className="gap-4 p-4 md:px-6 lg:px-8 xl:px-10 md:gap-8 grid grid-cols-2 md:grid-cols-11 lg:grid-cols-12">
        <p
          data-aos="fade-in-up"
      ...
      { propertyData.map((item, i) => (
          ...
          >
            <div
              onClick={() => filterDispatch({ type: "PROPERTY_TYPE", payload: item.type })}
